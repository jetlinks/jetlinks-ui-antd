<template>
  <a-card title="操作配置">
    ...
    <a-table :pagination="false" :rowKey="getRowKey" :columns="columns" :data-source="dataSource" bordered>
      <template v-for="col in ['name', 'action', 'describe']" #[col]="{ text, record }" :key="col">
        <div>
          <a-input
...
export default defineComponent({
  ...
  setup (props, { emit }) {
    ...
    const edit = (key: string) => {
      editableData[key] = _.cloneDeep(dataSource.value.filter((item: DataItem) => key === item.key)[0])
    }
    const save = (key: string) => {
      Object.assign(dataSource.value.filter((item: DataItem) => key === item.key)[0], editableData[key])
    ...
    const remove = (key: string) => {
      _.remove(dataSource.value, (n: DataItem) => {
        return n.key === key
      })
      emit('update:modelValue', dataSource.value)
    }
