<template>
  <page-container title="证书管理">
    <BaseCrud @event-change="eventChange" :form-data="formData" @modal-change="modalChange" @modal-visible="modalVisible" :model="model" :columns="columns" title="证书列表" :formPar="FormPar" :service="service" :default-params="defaultParams">
      <template #extra>
        <a-space>
...
export default defineComponent({
  ...
  setup () {
    const service = new BaseService<UserItem>('network/certificate')
    const columns = [
      {
        title: '名称',
        dataIndex: 'name',
      ...
      },
      {
        ...
        key: 'instance',
        searchParams: {
          transform: (array: string[]) => ({ instance$IN: array.join(',') })
        },
        search: {
          mode: 'multiple',
          type: 'select',
