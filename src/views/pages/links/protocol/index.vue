<template>
  <page-container title="协议管理">
    <BaseCrud @event-change="eventChange" :form-data="formData" @modal-change="modalChange" @modal-visible="modalVisible" :model="model" :columns="columns" title="协议列表" :formPar="FormPar" :service="service" :default-params="defaultParams">
      <template #extra>
        <a-space>
      ...
      <template #state="slotProps">
        <template v-if="slotProps.value === 1">
          <a-tag color="#108ee9">已发布</a-tag>
        </template>
        <template v-else>
          <a-tag color="#f50">未发布</a-tag>
        </template>
      </template>
...
export default defineComponent({
  ...
  setup () {
    const service = new BaseService<UserItem>('protocol')
    const columns = [
      {
        title: 'ID',
        dataIndex: 'id',
