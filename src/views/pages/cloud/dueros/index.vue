<template>
  <page-container title="DuerOS">
    <BaseCrud @event-change="eventChange" :form-data="formData" @modal-change="modalChange" @modal-visible="modalVisible" :model="model" :columns="columns" title="DuerOS列表" :formPar="FormPar" :service="service" :default-params="defaultParams">
      <template #extra>
        <a-space>
          <a-button @click="addBtn" type="primary">新增</a-button>
        </a-space>
      </template>
      <template #actionMappings="slotProps">
        <span>{{ slotProps.value.length }}</span>
      </template>
      <template #operation>
        <a-space>
          <a>编辑</a>
...
export default defineComponent({
  ...
  setup () {
    const service = new BaseService<UserItem>('dueros/product')
    const columns = [
      {
        title: 'ID',
        dataIndex: 'id',
