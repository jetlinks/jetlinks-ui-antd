...
const Play = (props: Props) => {
  ...
  useEffect(() => {
    ...
    setUrl(
      `/jetlinks/media/device/${props.data.deviceId}/${
        props.data.channelId
      }/live.mp4?:X_Access_Token=${getAccessToken()}`,
    );
    setProtocol('mp4');
    setBloading(false);
    // setUrlItem(res);
  ...
  const play = (value: string) => {
    setUrl(
      `/jetlinks/media/device/${props.data.deviceId}/${
        props.data.channelId
      }/live.${value}?:X_Access_Token=${getAccessToken()}`,
    );
    setProtocol(value);
  };
  ...
  const controlStart = (direct: string) => {
    if (playing) {
      service
        .getControlStart(props.data.deviceId, props.data.channelId, direct, 90)
        .subscribe(() => {});
    }
  };
  const controlStop = () => {
  ...
  const refresh = () => {
    ...
    setUrl(
      `/jetlinks/media/device/${props.data.deviceId}/${
        props.data.channelId
      }/live.${protocol}?:X_Access_Token=${getAccessToken()}&time=${new Date().getTime()}`,
    );
      ...
      <div className={styles.player_box}>
        <div className={styles.player_left}>
                ...
                <div
                  ...
                  onClick={() => {
                    if (isStart) {
                      setIsStart(false);
                      service
                        .endVideo(props.data.deviceId, props.data.channelId, { local: false })
                        .subscribe(resp => {
                          if (resp.status === 200) {
                            setVideo(false);
                            setIsStart(true);
                ...
                <div
                  ...
                  onClick={() => {
                    if (isStart) {
                      setIsStart(false);
                      service
                        .startVideo(props.data.deviceId, props.data.channelId, { local: false })
                        .subscribe(resp => {
                          if (resp.status === 200) {
                            setVideo(true);
                            setIsStart(true);
          ...
          <div className={styles.bottom}>
            ...
            >
              {playerBtnGroup.length > 0 &&
                playerBtnGroup.map((item, index) => (
        ...
        <div className={styles.player_right}>
          <div className={styles.ptz_block}>
            ...
            >
              {playing && playUp ? (
                <img src="/img/up.svg" width="30px" />
              ) : (
                <img src="/img/up_1.svg" width="30px" />
              )}
            </div>
            <div
              className={styles.ptz_left}
              title="左"
              onMouseDown={() => {
                controlStart('LEFT');
                setPlayLeft(false);
            ...
            >
              {playing && playLeft ? (
                ...
                <img src="/img/left_1.svg" width="30px" />
              )}
            </div>
            <div className={styles.ptz_center} title="云控制台">
              {playing && playCenter ? (
                <img src="/img/audio.svg" width="30px" />
              ) : (
                <img src="/img/audio_1.svg" width="30px" />
              )}
            </div>
            <div
              className={styles.ptz_right}
              title="右"
              onMouseDown={() => {
                controlStart('RIGHT');
                setPlayRight(false);
            ...
            >
              {playing && playRight ? (
                ...
                <img src="/img/right_1.svg" width="30px" />
              )}
            </div>
            <div
              className={styles.ptz_down}
              title="下"
              onMouseDown={() => {
                controlStart('DOWN');
                setPlayDown(false);
            ...
            >
              {playing && playDown ? (
                <img src="/img/down.svg" width="30px" />
              ) : (
                <img src="/img/down_1.svg" width="30px" />
              )}
            </div>
            <div
              className={styles.ptz_zoomin}
              title="放大"
              onMouseDown={() => {
            ...
            >
              {playing && playIn ? (
                ...
                <img src="/img/add_1.svg" width="30px" />
              )}
            </div>
            <div
              className={styles.ptz_zoomout}
              title="缩小"
              onMouseDown={() => {
            ...
            >
              {playing && playOut ? (
                <img src="/img/sub.svg" width="30px" />
              ) : (
                <img src="/img/sub_1.svg" width="30px" />
              )}
            </div>
          </div>
        </div>
