...
const Play = (props: Props) => {
    ...
    useEffect(() => {
        ...
        setUrl(`/jetlinks/media/device/${props.data.deviceId}/${props.data.channelId}/live.mp4?:X_Access_Token=${getAccessToken()}`);
        setProtocol('mp4');
        setBloading(false);
        // setUrlItem(res);
    ...
    const play = (value: string) => {
        setUrl(`/jetlinks/media/device/${props.data.deviceId}/${props.data.channelId}/live.${value}?:X_Access_Token=${getAccessToken()}`);
        setProtocol(value)
    };
    ...
    const refresh = () => {
        ...
        // service.getPlay(props.data.deviceId, props.data.channelId).subscribe(res => {
        // setUrl(res[protocol]);
        setUrl(`/jetlinks/media/device/${props.data.deviceId}/${props.data.channelId}/live.${protocol}?:X_Access_Token=${getAccessToken()}`);
                    ...
                    <div className={styles.bottom}>
                        <Radio.Group defaultValue="mp4" buttonStyle="solid" onChange={(e) => {
                            ...
                            {playerBtnGroup.length > 0 && playerBtnGroup.map((item, index) => (
                                <Radio.Button key={index} value={item.value}>{item.name}</Radio.Button>
                            ))}
                        </Radio.Group>
                    </div>
                </div>
