...
const RuleFlow: React.FC<Props> = (props) => {
    ...
    const [modelData, setModelData] = useState(initState.modelData);
    ...
    return (
        <Drawer
            title={`${modelData.option === 'update' ? '编辑' : modelData.option === 'copy' ? '复制' : '新增'}规则模型`}
            width="80vw"
            placement="right"
            visible
            onClose={() => props.close()}
