...
const oneLinkList: React.FC<{}> = () => {
  ...
  const handleSearch = (params?: any) => {
    ...
    apis.ctwingCmp.list(encodeQueryParam(params))
      .then((response: any) => {
  ...
  const deleteItem = (id: string) => {
    apis.ctwingCmp.remove(id).then(res => {
      if (res.status === 200) {
  ...
  const setEnabled = (id: string) => {
    apis.ctwingCmp.save(id, { id, state: 'enabled' }).then(res => {
      if (res.status === 200) {
  ...
  const setDisabled = (id: string) => {
    apis.ctwingCmp.save(id, { id, state: 'disabled' }).then(res => {
      if (res.status === 200) {
  ...
  return (
    <PageHeaderWrapper title="电信CTWingCmp">
