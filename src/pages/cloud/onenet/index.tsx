...
const OneNet: React.FC<{}> = () => {
    ...
    const handleSearch = (params?: any) => {
        ...
        apis.onenet.list(encodeQueryParam(params))
            .then((response: any) => {
    ...
    const deleteOneNet = (id: string) => {
        apis.onenet.remove(id).then(res => {
            if (res.status === 200) {
    ...
    const setEnabled = (id: string) => {
        apis.onenet.setEnabled(id).then(res => {
            if (res.status === 200) {
    ...
    const setDisabled = (id: string) => {
        apis.onenet.setDisabled(id).then(res => {
            if (res.status === 200) {
                        ...
                        record.state.value === 'disabled' && (
                            <>
                                ...
                                <Popconfirm title="确认删除？"
                                    onConfirm={() => { deleteOneNet(record.id); }}
                                >
                                    <a>删除</a>
                                </Popconfirm>
                            </>
    ...
    return (
        <PageHeaderWrapper title="移动OneNet">
