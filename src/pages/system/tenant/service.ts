...
class Service extends BaseService<TenantItem> {
  ...
  public create = (params: any) =>
    defer(() =>
      from(
        request(`/jetlinks/tenant/_create`, {
          method: 'POST',
          data: params,
        }),
  ...
  public list = (params: any) =>
    defer(() =>
      from(
        request(`/jetlinks/tenant/detail/_query`, {
          method: 'GET',
          params,
        }),
      ).pipe(
        ...
        map(result => {
          const temp = result;
          temp.data = result.data.map((i: any) => ({ members: i.members, ...i.tenant }));
          return temp;
        }),
      ),
    );
