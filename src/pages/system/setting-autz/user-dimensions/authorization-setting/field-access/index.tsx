...
const FieldAccess: React.FC<Props> = (props) => {
    ...
    const buildAccess = () => {
        ...
        tempFieldAccess.forEach(item => {
            item.actions.forEach(ac => {
                ...
                //获取不到就创建
                let fieldAccess = tempAccess.get(ac.action) || tempAccess.set(ac.action, {
                    action: ac.action,
                    type: 'DENY_FILEDS',
                    config: {
                        fields: []
    ...
    return (
        <Modal
            ...
            visible
            width={800}
            onCancel={() => { props.close(); setCurrentItem({}) }}
            onOk={() => { buildAccess() }}
        >
            <Alert
                            ...
                            return (
                                <Row style={{ width: '100%', marginTop: 20 }} key={data.name} id={data.name}>
                                    <Col span={4} style={{ fontWeight: 700 }}>
                                        <span style={{ fontSize: '14px', color: '#f5222d' }}>* </span>{data.describe}
                                    </Col>
                                    <Col span={20}>
                                        <Checkbox.Group style={{ width: '100%' }} value={data.actions.filter(e => e.checked === true).map(e => e.action)}>
                                            {
                                                data.actions.map(item =>
                                                    <Col key={item.action} span={5}><Checkbox value={item.action} onClick={() => { checkItem(data.name, item.action) }}>{item.name}</Checkbox></Col>
                                                )
                                            }
                                        </Checkbox.Group>
                                    </Col>
