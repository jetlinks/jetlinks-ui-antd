...
const Save: React.FC<Props> = (props) => {
    ...
    const save = () => {
        form.validateFields((err, fieldValue) => {
            ...
            apis.dimensions.addDimensionUser(param).then(e => {
                if (e.status === 200) {
        ...
        return params.userId
            ...
            .flatMap(u => {
                return params.dimensions
                    .map(did => {
                        const dimension: any = dimensionList
                            .find(dimension => dimension.id === did);
                        return {
    ...
    const renderDimensions = () => {
        return (
            <Checkbox.Group>
                ...
                {typeList.map((type: DimensionType) =>
                    ...
                    <Card bordered={false} key={type.id} style={{ width: 600 }}>
                        ...
                        {
                            dimensionList.filter((i: DimensionsItem) => i.typeId === type.id).map((item: DimensionsItem) => {
                                return <Col span={7} key={item.id}><Checkbox value={item.id}>{item.name}</Checkbox></Col>
                            })
                        }
                    </Card>
                )}
