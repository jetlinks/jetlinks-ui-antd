...
const RoleModel: RoleModelType = {
    ...
    effects: {
        *query({ payload, callback }, { call, put }) {
            const response: SimpleResponse = yield call(apis.role.list, payload);
            yield put({
                type: 'save',
        ...
        *insert({ payload, callback }, { call }) {
            const response: SimpleResponse = yield call(apis.role.add, payload);
            callback(response);
        },
        *remove({ payload, callback }, { call, put }) {
            const response: SimpleResponse = yield call(apis.role.remove, payload);
            callback(response);
        }
    },
    reducers: {
