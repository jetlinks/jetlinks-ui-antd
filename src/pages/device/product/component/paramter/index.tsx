import { Form, Input, Select, Col, Button, Radio, Drawer, Icon, Row, List } from "antd";
        ...
        switch (dataType) {
                ...
                return (
                    <div>
                        <Form.Item label="时间格式">
                            ...
                            <Select value={data.valueType.timeFormat} onChange={(value: string) => {
                                data.valueType = {
                                    timeFormat: value
                                };
                                setData(data);
            ...
            case 'object':
                ...
                if (data.valueType.parameters) {
                    // console.log(data.valueType.parameters, '1');
                    // console.log(data.valueType.parameters.parameters, '2');
                    parameters = data.valueType.parameters;
                    // parameters.map(item => {
                    //     return <Input value={item.name + '--' + item.mark} />
                    // })
                }
                return (
                    <Form.Item label='JSON对象'>
                        {
                            console.log(data.valueType.parameters, 'lengths')
                            /* {
                            (data.valueType.parameters) ? parameters.map((item, index) => {
                                return <Input key={index} value={item.name + '--' + item.mark} />
                            }) : ''} */
                        }
                        {
                                        ...
                                        <List.Item
                                            actions={[
                                                ...
                                                <Button type="link" onClick={() => {
                                                    const index = data.valueType.parameters.findIndex((i: any) => i.id === item.id);
                        ...
                        <Button type="link" onClick={() => { setParameterVisible(true) }}><Icon type='plus' />添加参数</Button>
                    </Form.Item>
                );
            default:
                return;
        ...
        >
            <Form className={styles.paramterForm}>
                ...
                <Form.Item label='数据类型'>
                    <Select
                        ...
                        <Select.OptGroup label="基本类型">
                            ...
                            <Select.Option value="text">text(字符串)</Select.Option>
                ...
                <Form.Item label="设备上报">
                    <Radio.Group value={data.report} onChange={e => {
                        data.report = e.target.value;
                        setData({ ...data });
                    }}>
                        <Radio value={true}>是</Radio>
                        <Radio value={false}>否</Radio>
            ...
            </div>
            {
                ...
                <Paramter
                    ...
                    save={(item) => {
                        ...
                        index === -1 ? data.valueType.parameters.push(item) : data.valueType.parameters[index] = item;
                        setData({ ...data });
                        // props.close();
                    }}
